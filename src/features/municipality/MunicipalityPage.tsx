...
export function MunicipalityPage({ data, session }: MunicipalityPageProps) {
  ...
  return <main className="municipality-page" id="main-content">
    ...
    <nav className="municipality-shortcuts" aria-label="Municipality sections">
      ...
      {citizenView ? <Link href="/wards">Go to your ward <ArrowUpRight size={16} aria-hidden="true" /></Link> : <a href="#municipal-ward-register">Find a ward</a>}
      {!citizenView ? <a href="#municipality-details">Municipality details</a> : null}
    </nav>
    {!citizenView ? <section id="municipality-details" className="municipality-section" aria-labelledby="municipality-details-title"><div className="municipality-section-heading"><div><p className="municipality-kicker">At a glance</p><h2 id="municipality-details-title">Municipality details</h2></div></div><dl className="municipality-facts"><div><dt>District</dt><dd>{data.municipality.district}</dd></div><div><dt>State</dt><dd>{data.municipality.state}</dd></div><div><dt>Ward count</dt><dd>{data.wards.length}</dd></div><div><dt>Total allocation</dt><dd>{formatRupees(totalAllocated)}</dd></div></dl></section> : null}
